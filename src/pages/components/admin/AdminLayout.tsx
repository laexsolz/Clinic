...
export default function AdminLayout({ children, initialView = 'dashboard', view: controlledView, onViewChange }: Props) {
  ...
  const [internalView, setInternalView] = useState<AdminView>(initialView);
  ...
  const ActiveContent = useMemo(() => {
    switch (view) {
      ...
      case 'billing':
        return <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">Billing panel (demo)</div>;
      case 'settings':
        return <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">Settings panel (demo)</div>;
      case 'audit':
        return <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">Audit logs (demo)</div>;
      default:
        return <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">Dashboard (demo)</div>;
    }
  }, [view]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      <AdminTopNav onToggle={handleTopNavToggle} />
